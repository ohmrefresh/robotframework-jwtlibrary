...
Examples:
  ...
  jwt-robot-tool compare --token1 <token1> --token2 <token2>
    ...
    generate_parser = subparsers.add_parser(
        'generate', 
        help='Generate a JWT token'
    )
    generate_parser.add_argument(
        '--payload', 
        required=True,
        help='JSON payload for the token'
    )
    generate_parser.add_argument(
        '--secret', 
        required=True,
    ...
    decode_parser.add_argument(
        '--secret',
        help='Secret key for verification (optional for unsafe decode)'
    )
    decode_parser.add_argument(
        '--algorithm',
        default='HS256',
    ...
    validate_parser.add_argument(
        '--secret', 
        required=True,
        help='Secret key for validation'
    )
    validate_parser.add_argument(
        '--algorithm',
        default='HS256',
    ...
    info_parser = subparsers.add_parser(
        'info', 
        help='Get JWT token information'
    )
    ...
    compare_parser = subparsers.add_parser(
        'compare', 
        help='Compare two JWT tokens'
    )
    compare_parser.add_argument(
        '--token1', 
        required=True,
        help='First JWT token'
    )
    compare_parser.add_argument(
        '--token2', 
        required=True,
    ...
    verify_parser = subparsers.add_parser(
        'verify', 
        help='Verify JWT token signature and expiration'
    )
...
def command_validate(args, jwt_lib: JWTLibrary) -> int:
    ...
    try:
        ...
        print(f"  Expiration: {'Valid' if not exp_info.get('is_expired', False) else 'Expired'}")
        if args.expected_claims:
