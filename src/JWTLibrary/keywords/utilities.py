...
class UtilityKeywords:
    ...
    def create_jwt_payload(self, **kwargs) -> Dict[str, Any]:
        ...
        Examples:
        | ${payload}=    Create JWT Payload    user_id=123    role=admin    email=user@example.com
        """
        payload = dict(kwargs)
    ...
    def compare_jwt_tokens(self, token1: str, token2: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for key in all_header_keys:
                ...
                if val1 != val2:
                    header_differences[key] = {
                        'token1': val1,
                        'token2': val2
                    }
            
            are_identical = len(payload_differences) == 0 and len(header_differences) == 0
            ...
            logger.info(f"JWT token comparison: {'Identical' if are_identical else 'Different'}")
    ...
    def extract_jwt_timestamps(self, token: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if 'exp' in timestamps:
                ...
                timestamps['expires_in_seconds'] = (exp_dt - current_time).total_seconds()
    ...
    def generate_future_timestamp(self, hours: int = 24, minutes: int = 0, seconds: int = 0) -> int:
        """
        ...
        """
        future_time = datetime.utcnow() + timedelta(hours=hours, minutes=minutes, seconds=seconds)
        timestamp = int(future_time.timestamp())
        logger.info(f"Generated future timestamp: {timestamp} ({hours}h {minutes}m {seconds}s from now)")
        return timestamp
    ...
    def format_jwt_claims_for_logging(self, claims: Dict[str, Any], 
        ...
        try:
            ...
            logger.debug(f"Formatted JWT claims for logging")
            return formatted
    ...
    def validate_jwt_claim_types(self, claims: Dict[str, Any]) -> Dict[str, Any]:
            ...
            if claim_name in type_expectations:
                ...
                if not isinstance(claim_value, expected_type):
                    validation_result['errors'].append(
                        f"Claim '{claim_name}' should be {expected_type}, got {type(claim_value)}"
                    )
        ...
        logger.info(f"JWT claim type validation: {'Valid' if validation_result['is_valid'] else 'Invalid'}")
